...
function test() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let info;
        for (let i = 0; i < 1; i++) {
            for (let j = listToken.length - 2; i < listToken.length; j++) {
                pair = yield swapUtil.FactoryContract.getPair(listToken[i].address, listToken[j].address);
                ...
                if (!swapUtil.isZeroAddress(pair)) {
                    ...
                    info = yield swapUtil.getInformationFromInput(listToken[i].address, listToken[j].address, 0.5 / 100, "0");
                }
            }
        }
