...
import { TravaAuctionFinalizeAuction } from "../../../src/actions/trava/nft/marketplace/auction/TravaAuctionFinalizeAuction";
async function test() {
    ...
    const trava_finalize_auction = new TravaAuctionFinalizeAuction("128" , "0x2b3fBc2c23E300C6A188e790E59668D501F2568a");

    const recipe = new Recipe("finalize" , 97 , [trava_finalize_auction]);
    ...
    let tx = await proxyContract["execute(address,bytes)"](encoded[0] as string, encoded[1] as unknown as string[]);
    await tx.wait();
